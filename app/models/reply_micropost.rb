...
class ReplyMicropost < ActiveRecord::Base
  ...
  PER_PAGE = 10
  PRAISE = {:NOKUDOS =>0,:KUDOS =>1}
  PRAISE_NAME = {0=>'没被赞',1=>'被赞'}

  #获取子消息（分页）
  def self.get_microposts micropost_id, page
    base_sql = "select r.id, r.content, r.sender_id, r.sender_types, r.reciver_id, r.praise, DATE_FORMAT(r.created_at, '%Y-%m-%d %H:%i:%S') as new_created_at, s.name sender_name,
              s.avatar_url sender_avatar_url, u.name reciver_name, u.avatar_url reciver_avatar_url,r.praise
